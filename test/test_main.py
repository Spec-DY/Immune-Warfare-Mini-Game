...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, keys):
        ...
        if keys[pygame.K_LEFT] and self.rect.left > 0:
            ...
            print("player left moved")
        if keys[pygame.K_RIGHT] and self.rect.right < SCREEN_WIDTH:
            self.rect.x += self.speed
            print("player right moved")
...
class EnemyBullet(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.top > SCREEN_HEIGHT:
            self.kill()
            print("Boss bullet out of screen")
...
def startmanu():
    ...
    """
    print("Startnamu function called")
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if start_button.is_clicked(pygame.mouse.get_pos()):
                    print("startmanu function return")
                    return
        screen.blit(startimage, (0, 0))
        start_button.drawbutton()
        pygame.display.flip()
...
def boss_message():
    ...
    while True:
        ...
        print(f"Boss countdown seconds value:{seconds}")
        if seconds > countdown_time:
            print("Boss countdown finish")
            break  # countdown end
        screen.fill(BLACK)
...
def countdown_with_mission():
    ...
    """
    print("countdown_with_mission called sucessfully")
    countdown_time = 3
    start_ticks = pygame.time.get_ticks()  # start time
    ...
    while True:
        seconds = (pygame.time.get_ticks() - start_ticks) / \
            1000  # calculate time passed
        if seconds > countdown_time:
            print("Game start countdown finish")
            break  # countdown end
        screen.fill(BLACK)
...
def end_game(score, game_over, boss_health):
    ...
    if game_over:
        print("Now the game should be over")
        if boss_health < 1:
            print("You should see the win image")
            screen.blit(winimage, (0, 0))
            screen.blit(win_text, win_rect)
            viruskilled = font.render(
                ENDGAME_KILLED.format(score), True, BLACK)  # black text
            print(f"you killed{score}virus")
        else:
            print("You should see the lose image")
...
def score_display(score):
    ...
    """
    print(f"score_display called, score passed in is{score}")
    score_text = font.render(INGAME_SCORE.format(score), True, WHITE)
    screen.blit(score_text, (10, 10))
    print("score_display done")
...
def timer_display(time_remaining):
    ...
    """
    print(f"timer_display called time remaining passed in is{time_remaining}")
    timer_text = font.render(INGAME_TIMER.format(time_remaining), True, WHITE)
    screen.blit(timer_text, (SCREEN_WIDTH - 150, 10))
    print("timer_display done")
...
def clean_stack():
    ...
    """
    print("clean_stack called, cleaning now...")
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            exit()
...
def main():
    ...
    while running:
        ...
        if not game_over and time_remaining == 0 and score < MIN_SCORE:  # low score fail
            print("score lower than min_score game over triggered")
            game_over = True
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if restart_button.is_clicked(mouse_pos):
                    print("game restarted, all initialized")
                    main()
        ...
        if boss_fight:
            ...
            if player_hit:  # hit by boss
                ...
                if player.health <= 0:
                    print("player dead trigger game over with lose image")
                    game_over = True
                    boss_fight = False
            ...
            if boss_hits:  # hit by player
                ...
                print("boss health -1")
                if boss.health <= 0:
                    print("boss dead trigger game over with win image")
                    game_over = True
                    boss_fight = False
